...
function makeDefaults(html, userID, ctx) {
	let reqCounter = 1;
	let fb_dtsg = getFrom(html, 'name="fb_dtsg" value="', '"');
	if (!fb_dtsg) fb_dtsg = getFrom(html, '"DTSGInitData",[],{"token":"', '"');
	let ttstamp = "2";
...
function parseAndCheckLogin(ctx, defaultFuncs, retryCount = 0, sourceCall) {
	if (sourceCall === undefined) {
		try { throw new Error(); } catch (e) { sourceCall = e; }
	}
	return function (data) {
		return tryPromise(function () {
			...
			if (data.statusCode >= 500 && data.statusCode < 600) {
				...
				log.warn("parseAndCheckLogin", `Got status code ${data.statusCode} - Retrying in ${retryTime}ms...`);
