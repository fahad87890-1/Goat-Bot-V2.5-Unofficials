...
// Auto-bypass scraping warning checkpoint if encountered
let isBehavior = false;
async function bypassAutoBehavior(resp, jar, globalOptions, appstate, ID) {
try {
	if (!resp || !resp.request || !resp.request.uri || !resp.request.uri.href) return resp;
	const href = resp.request.uri.href;
	if (href.includes("https://www.facebook.com/checkpoint/") && href.includes("601051028565049")) {
		const appstateCUser = (Array.isArray(appstate) ? appstate.find(i => i.key == 'c_user') || appstate.find(i => i.key == 'i_user') : null);
		...
		return utils.post("https://www.facebook.com/api/graphql/", jar, {
			...
			.then(res => {
				try { log.info("login", "Bypassed Facebook scraping warning successfully."); } catch (_) { }
				return res;
			})
			.catch(() => {
				try { log.warn("login", `${UID || 'Unknown UID'} may have automated behavior warning.`); } catch (_) { }
	...
	try { log.error("login", "Error while bypassing Facebook warning.", e); } catch (_) { }
	return resp;
}
}
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
	return function (res) {
		...
		// Getting cookies from the HTML page
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
	...
	function checkBrowserSupport(res) {
	...
	mainPromise = mainPromise
		...
		.then(handleRedirect)
		.then(function (res) { return bypassAutoBehavior(res, jar, globalOptions, appState); })
		.then(function (res) {
			const html = res.body;
