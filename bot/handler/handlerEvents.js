...
module.exports = function (api, threadModel, userModel, globalModel, usersData, threadsData, globalData) {
	return async function (event, message) {
		...
		*/
		async function onAnyEvent() {
			...
			// Debug: show core toggles for visibility on first few events
			try {
				const fca = GoatBot.config.optionsFca || {};
				utils.log.info('EVENT', utils.jsonStringifyColor({ type: event.type, listenTyping: !!fca.listenTyping, selfListen: !!fca.selfListen, selfListenEvent: !!fca.selfListenEvent }, null, 2));
			} catch (_) {}
		...
		*/
		async function presence() {
			try {
				const fca = GoatBot.config.optionsFca || {};
				if (fca.updatePresence) utils.log.info('PRESENCE', utils.jsonStringifyColor(event, null, 2));
			} catch (_) {}
		}
		...
		*/
		async function typ() {
			try {
				const fca = GoatBot.config.optionsFca || {};
				if (fca.listenTyping) utils.log.info('TYP', utils.jsonStringifyColor({ from: event.from, to: event.to || event.threadID, isTyping: !!(event.st || event.isTyping) }, null, 2));
			} catch (_) {}
		}
